...
class TetrisView: View {
   ...
   private class ViewHandler(private val owner: TetrisView) : android.os.Handler(){
            ...
            if (message.what == 0) {
                if (owner.model != null) {
                    if (owner.model!!.isGameOver()) {
                        owner.model?.endGame()
                        Toast.makeText(owner.activity, "Конец игры", Toast.LENGTH_LONG).show();
                    }
    ...
    private fun drawFrame(canvas: Canvas) {
        ...
        canvas.drawRect(frameOffset.width.toFloat(), frameOffset.height.toFloat(), width - frameOffset.width.toFloat(),
